...
def movimiento():
    ...
    for i in posicion_snake:
        if len(posicion_snake)==7000:
        	maximo_nivel()     

    cantidad.after(300, movimiento)

def cruzar_snake():
    mixer.music.load("audio_dead.mp3")
    mixer.music.play(loops=0)
    canvas.delete(ALL)
    canvas.create_text(canvas.winfo_width() / 2, canvas.winfo_height() / 2, 
        
        text=f"Fin del juego :( \n\n haz perdido u.u 🍎",fill='red',
...
ventana.title('Snake.io')
ventana.geometry('887x615')
...
button3 = Button(frame_1, text='Re-intentar', bg='blue' ,
    command = movimiento)
...
cantidad =Label(frame_1, text='SCORE 🍎 : ', bg='black', 
